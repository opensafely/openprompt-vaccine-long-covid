...
vaccine_data <- lc_first %>% 
  dplyr::select(patient_id, no_post_lc_vacc, first_lc, starts_with("pt_"), sex, age_cat) %>% 
  mutate(
...
ggplot(lc_first_vacced, aes(x = no_prev_vacc, y = vacc_prev, fill = lc_out, colour = lc_out)) + 
