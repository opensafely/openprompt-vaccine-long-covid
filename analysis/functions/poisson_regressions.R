...
poisson_regressions <- function(cohort_data, stratifier) {
  ...
  fm2 <- formula(paste("lc_out ~ offset(log(t)) + ", stratifier, "+ age_centred + sex"))
  ...
  calculate_predictions <- function(model){
    ...
    model %>% 
      ...
      dplyr::select(term, estimate, conf.low, conf.high,
                    predicted_vals, lci, uci)
  }
  
  poissonmodel_crude <- glm(fm1, data=cohort_data, family = poisson(link = "log")) 
  crude_output <- calculate_predictions(poissonmodel_crude)
    
  poissonmodel_adj <- glm(fm2, data=cohort_data, family = poisson(link = "log")) 
