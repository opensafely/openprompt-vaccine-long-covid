...
create_forest_plot <- function(data_in, y_col_var, plot_rel_widths = c(7, 3), legend_position = "right") {
  ...
  cowplot::plot_grid(rr_tab, rr_forest, rel_widths = plot_rel_widths)
}

# Crude RRs
pdf(here("output/figures/fig3a_crude_RRs.pdf"), width = 20, height = 20)
  create_forest_plot(filter(full_rates, model == "crude"), y_col_var = "outcome")
dev.off()

# Adjusted RRs
pdf(here("output/figures/fig3b_adjusted_RRs.pdf"), width = 20, height = 20)
  create_forest_plot(filter(full_rates, model == "adjusted"), y_col_var = "outcome")
dev.off()

# Long COVID adjusted outcomes only
pdf(here("output/figures/fig3c_longcovid_RRs.pdf"), width = 20, height = 20)
...
pdf(here("output/figures/fig3h_rate_ratios_facet.pdf"), width = 12, height = 14)
pd = position_dodge(1)
adjusted_rates_out %>% 
  filter(plot_marker) %>% 
