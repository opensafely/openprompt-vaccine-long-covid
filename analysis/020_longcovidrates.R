...
df_vacc_base <- df_full %>% 
  filter(t>0) ## remove anyone who starts and ends on the same date 
...
df_vacc_long$t[df_vacc_long$t < 0] <- 0 # if this gap is negative then the vaccine was delivered pre-study enrol so set t = 0
